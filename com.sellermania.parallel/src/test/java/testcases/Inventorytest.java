...
public class Inventorytest extends driverinitialization {
	...
	public void inventorypage() throws InterruptedException {
		...
		String actual = driverinitialization.getDriver().getCurrentUrl();
		String expected = "/inventory";

		try {

			if (actual.contains(expected)) {
				System.out.println("inventory mnu opened" + driverinitialization.getDriver().getCurrentUrl());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

		JavascriptExecutor js = (JavascriptExecutor) driverinitialization.getDriver();
//	js.executeScript("window,scrollBy(0,500)");
		...
		ip.Listing("sku_test_501", 5, "test", 100, 50, 2);
